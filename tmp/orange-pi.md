...
flowchart TB
  subgraph Host["Host (Dev machine)"]
    A1[OpenPLC Editor<br/>(IDE)]
    A2[SCADA / Ignition / Node-RED]
    A3[Modbus Simulator<br/>(pyModbus/pymodbus)]
    A4[GPS Simulator<br/>(NMEA → PTY)]
    A5[MQTT / REST Server<br/>(Postgres)]
  ...
  subgraph QEMU_OPI["QEMU: OrangePi Zero2 (emulated)"]
    Q1[Linux rootfs]
    Q2[OpenPLC Runtime]
    Q3[Serial devices (/dev/ttyS* mapped)]
    Q4[Modbus Client/Server]
    Q5[Data Aggregator (GPS filter, encoder)]
    Q6[Service: uploader → Remote Server (HTTP/MQTT)]
  end
